...
class PicturePerfect extends StatefulWidget {
  const PicturePerfect({super.key, required this.eventName});
  final String eventName;

  @override
  State<PicturePerfect> createState() => _PicturePerfectState();
}

final String? uid = AuthService().getCurrentUid();

class _PicturePerfectState extends State<PicturePerfect> {
  ...
  void loadSubmittedText() async {
    ...
    setState(() {
      submittedTextPerfect = prefs.getString('submittedTextPerfect') ?? '';
    });
  }

  void saveSubmittedText(String text) async {
    final prefs = await SharedPreferences.getInstance();
    prefs.setString('submittedTextPerfect', text);
  }
  ...
  Future<void> uploadFile(String uid, String eventName) async {
    ...
    final path = 'PicturePerfect/${pickedFile!.name}';
    ...
    try {
      ...
      setState(() {
        submittedTextPerfect = 'Submitted';
        saveSubmittedText(submittedTextPerfect);
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    if (submittedTextPerfect.isNotEmpty) {
