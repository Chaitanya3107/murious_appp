...
class FrameByFrame extends StatefulWidget {
  const FrameByFrame({super.key, required this.eventName});
  final String eventName;

  @override
  State<FrameByFrame> createState() => _FrameByFrameState();
}

final String? uid = AuthService().getCurrentUid();

class _FrameByFrameState extends State<FrameByFrame> {
  ...
  void loadSubmittedText() async {
    ...
    setState(() {
      submittedTextFrame = prefs.getString('submittedTextFrame') ?? '';
    });
  }

  void saveSubmittedText(String text) async {
    final prefs = await SharedPreferences.getInstance();
    prefs.setString('submittedTextFrame', text);
  }
  ...
  Future<void> uploadFile(String uid, String eventName) async {
    ...
    try {
      ...
      setState(() {
        submittedTextFrame = 'Submitted';
        saveSubmittedText(submittedTextFrame);
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    if (submittedTextFrame.isNotEmpty) {
